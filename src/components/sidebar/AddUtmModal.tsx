...
export const AddUtmModal: React.FC<ModalType> = ({ isOpen, onRequestClose, style }) => {
  const {
    register,
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      setError("utm_url", { message: "UTM을 확인해주세요" }, { shouldFocus: true });
    }
  };
  ...
  return (
    <>
      {excel ? (
        <ExcelAddModal isOpen={isOpen} onRequestClose={onRequestClose} style={style} />
      ) : (
        <Modal isOpen={isOpen} onRequestClose={onRequestClose} style={style}>
          <form onSubmit={handleSubmit(onSubmit)} className={styles.add_modal}>
            ...
            <span className={styles.sub_title}>*만드신 UTM을 넣으시면 파라미터 값이 분류되어 데이터에 저장됩니다</span>
            <div className={styles.modal_footer_box}>
              <div className={styles.error_box}>
                {errors?.utm_url && <span className={styles.error_utm}>추가하실 UTM을 확인해주세요!</span>}
              </div>
              ...
              <div className={styles.error_box_memo}>
                {errors.created_at && <span className={styles.error_utm}>추가하실 UTM의 날짜를 입력해주세요!</span>}
              </div>
              <button type="button" className={styles.add_excel_button} onClick={() => setExcel(true)}>
                엑셀로 추가하기
              </button>
              <button id="add_btn" type="submit" disabled={isSubmitting} className={styles.add_button}>
                추가하기
              </button>
            </div>
          </form>
